...
def _front_matter(token: Token, env: EnvType) -> L10NResult:
    domain_g: DomainGenerationProtocol = env['domain_generation']
    fm_result = domain_g.render_front_matter(token.content, token.markup)
    return fm_result


def _link_ref(env: EnvType, md_ctx: _MdCtx, content_result: L10NResult):
...
class RendererMarkdownL10N:
    ...
    def render(self, tokens: Sequence[Token], _options: OptionsDict, env: EnvType) -> Tuple[L10NResult, L10NResult]:
        ...
        if tokens[0].type == 'front_matter':
            fm_result = _front_matter(tokens[0], env)
            tokens = tokens[1:]
        else:
        ...
        _link_ref(env, md_ctx, content_result)
    ...
    @classmethod
    def blockquote_close(cls, _tokens: Sequence[Token], _idx: int, md_ctx: _MdCtx, content_result: L10NResult):
